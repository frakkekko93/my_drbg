...
pub fn run_demo() {
    ...
    println!("you can request prediction resistance at any time during bit generation.");
    println!("The DRBGs are also designed to have a reseed counter that allows for a");
    println!("limited number of consecutive generations without accessing the entropy");
    println!("source for fresh entropy. Once this limit has been reached, the DRBG will");
    println!("handle the reseeding by itself and you will be able to continue using the");
    println!("active instance.");
    println!("***************************************************************************");
    ...
    while user_choice != 0 {
        ...
        println!("\nWhich security strength do you need? (must be <=256)");
        print!("\nYour choice: ");

        let strength = get_input();

        println!("\nWould you like to use a personalization string for the instantiation?\n\t1- Yes\n\t2- No");
        print!("\nYour choice: ");
